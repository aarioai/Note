...
#   moved to https://github.com/pytorch/pytorch
# Before installation:
# 
# change `option(USE_NATIVE_ARCH "Use -march=native" OFF)` to `option(USE_NATIVE_ARCH "Use -march=native" ON)` in `CMakeLists.txt`
...
```
** GPU ERR! Volatile GPU-Util turns to 0%. GPU Fan and Pwr turns to ERR! **
cuda version unmatched.
GTX1080Ti matches cuda 9.0



## Config
```yaml
# see detectron/core/config.py
MODEL:
...
TRAIN:
  ...
  DATASETS: ('coco_2014_train',)   # defined at datasets/dataset_catalog.py
  SCALES: (500,)
  MAX_SIZE: 833
...
CUDA_VISIBLE_DEVICES=0,2 python tools/train_net.py \
